...
**Цель:** каждая фракция определяет героя, юнитов и стартовую армию. Город заполняется юнитами выбранной фракции.

### 11.1 Данные

- [ ] `assets/data/factions.ron` — «Японская деревня» (Наруто ⚔2/🛡1/👁5) и «Самодельный штаб» (Егор ⚔3/🛡0/👁5)
- [ ] `FactionDef { id, name, hero, units, starting_army }` в `src/data/mod.rs`
- [ ] `units.ron` — оставить только нейтралов (гоблин, орк, тролль)
- [ ] `default.ron` — убрать `name`/`army` из `hero`, убрать `recruits` из `towns`

### 11.2 Модель и бой

- [ ] `Hero::attack`, `Hero::defense`
- [ ] `BattleState::attacker_attack/defense_bonus`
- [ ] Стартовый город заполняется юнитами фракции в `load_map_from_config`

### 11.3 Меню и UI

- [ ] Карточки фракций вместо карточек героев (портрет + характеристики + юниты)
- [ ] Портрет и характеристики в панели приключения

### 11.4 Тесты

- [ ] `load_factions_parses`, `faction_unit_ids_match_starting_army`, `neutrals_still_load`
- [ ] `attack_bonus_increases_damage`, `defense_bonus_reduces_damage`, `defense_never_below_one`

**Критерий готовности:** стартовый город и армия героя определяются выбранной фракцией, не картой.

---

## Этап 12 — Прогресс героя ⬜
